...
CANDIDATES = ["monday_sentiment", "pc_ratio_snap", "short_pct_float"]
TF = "__signed_power_p05"   # the transform monday_sentiment showed signal on
...
for HORIZON in (3, 5):
    ...
    print(f"\n{'='*66}\nHORIZON {HORIZON}d  TRANSFORM={TF}  vs momentum: {[c.replace(TF,'') for c in mom_cols]}\n{'='*66}")
    for base in CANDIDATES:
        ...
        if raw and res:
            ...
            verdict = "ADMIT" if (keep and maxcorr < 0.4) else \
                      "REDUNDANT (momentum)" if maxcorr >= 0.4 else "WEAKENS (residual collapses)"
            print(f"    max|corr|={maxcorr}  -> {verdict}")
        elif raw is None:
            print(f"    raw IC None — transform degenerate at this horizon")
