# events_risk.py  — CI-safe secrets + FINNHUB var alias + robust fetch
...
# Map primary → accepted env alias list
_ENV_ALIASES = {
    "FMP_API_KEY": ["FMP_API_KEY"],
    "FINNHUB_TOKEN": ["FINNHUB_TOKEN", "FINNHUB_API_KEY"],  # accept either
}

def _get(key_env: str, *secret_path) -> Optional[str]:
    """
    Secrets loader:
      1) ENV (with alias support)
      2) st.secrets (only outside CI; supports nested paths)
    """
    # ENV first (with aliases)
    for k in _ENV_ALIASES.get(key_env, [key_env]):
        v = os.getenv(k)
        if v:
            return v

    # Never import Streamlit in CI (avoids TOML parse)
    if os.getenv("GITHUB_ACTIONS") == "true":
        return None

    # Optional: nested secrets via Streamlit when running the app
    try:
...
def fetch_economic_fmp(start: date, end: date) -> pd.DataFrame:
    ...
    try:
        ...
        print(f"⚠️ FMP econ fetch failed: {e}")
        return pd.DataFrame()


def fetch_earnings_finnhub(start: date, end: date) -> pd.DataFrame:
    tok = _get("FINNHUB_TOKEN", "providers", "finnhub_token")  # accepts FINNHUB_API_KEY too
    if not tok:
        return pd.DataFrame()
    try:
        ...
        for it in items:
            ...
            if time_hint == "amc":
                when = base.replace(hour=16, minute=0)
            elif time_hint == "bmo":
                when = base.replace(hour=8,  minute=0)
            else:
                when = base.replace(hour=13, minute=0)
            sym = it.get("symbol", "")
            rows.append({
        ...
        print(f"⚠️ Finnhub earnings fetch failed: {e}")
        return pd.DataFrame()
...
def fetch_ipo_finnhub(start: date, end: date) -> pd.DataFrame:
    ...
    try:
        ...
        print(f"⚠️ Finnhub IPO fetch failed: {e}")
        return pd.DataFrame()
...
def load_events(start: date, end: date, use_fmp: bool=True, use_finnhub: bool=True) -> pd.DataFrame:
    dfs = []
    if use_fmp:
        dfs.append(fetch_economic_fmp(start, end))
    if use_finnhub:
