...
DB="p0-2_A1.db"; PARQUET="p0-2_A1_panel.parquet"; PREFIX="p0-2_A1"
H=5; FOLDS=5; EMBARGO=5

print(f"=== P0-2 A1 START {time.strftime('%Y-%m-%d %H:%M:%S')} ===", flush=True)
...
print(f"=== P0-2 A1 DONE {time.strftime('%Y-%m-%d %H:%M:%S')} ===", flush=True)
