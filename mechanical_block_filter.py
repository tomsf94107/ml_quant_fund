...
# RULE C: known recon/rebalance dates. IMPORTANT: a date being on this list is
# NOT sufficient on its own to strip a day. Rule C only fires if the name ALSO
# had a corroborating large print that day (>= RULE_C_MIN_PRINT_USD). This stops
# a calendar date from wrongly stripping a name (e.g. MU) that traded normally
# through the rebalance and had no mechanical cross.
KNOWN_MECHANICAL_DATES: set[str] = {
    "2026-06-18",
    "2026-06-23",
}

# Rule C corroboration floor: on a known date, only strip if the day's max print
# is at least this large. Set well above routine block size but below the
# hundreds-of-millions recon crosses, so it confirms "yes, this name actually
# participated in the mechanical event" without catching normal blocks.
RULE_C_MIN_PRINT_USD = 50_000_000.0
...
def flag_mechanical(
    ...
    *,
    ...
    rule_c_min_print_usd: float = RULE_C_MIN_PRINT_USD,
) -> list[StripReason]:
    """Return a StripReason for each day judged mechanical (rules A/B/C).

    Rule C (known date) only fires WITH a corroborating large print, so a
    rebalance date never strips a name that traded normally through it.
    """
    ...
    for d in days:
        ...
        if d.max_print_usd >= abs_print_usd:
            fired.append("B")
        if d.day in known and d.max_print_usd >= rule_c_min_print_usd:
            fired.append("C")
        if fired:
