...
# FinBERT inference
# ------------------------------------------------------------------
def _finbert_polarity(texts: List[str], batch_size: int = 16) -> List[float]:
    """Run FinBERT in batches on CPU. batch_size=16 avoids OOM on large headline sets."""
    import os
    os.environ["TOKENIZERS_PARALLELISM"] = "false"  # silence HuggingFace warning
    torch.set_num_threads(1)                          # avoids OpenMP crash on macOS
    ...
    for i in range(0, len(texts), batch_size):
        ...
        batch = tok(
            ...
            max_length=128,          # 128 is enough for headlines, 512 is overkill
            return_tensors="pt"
        )
